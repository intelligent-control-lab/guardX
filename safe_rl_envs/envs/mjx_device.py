...
def _model_derived(value: mujoco.MjModel, device_id: int) -> Dict[str, Any]:
  return {k: jax.device_put(v, DEVICES[device_id]) for k, v in mesh.get(value).items()}


def _data_derived(value: mujoco.MjData, device_id: int) -> Dict[str, Any]:
  return {'efc_J': jax.device_put(value.efc_J, DEVICES[device_id])}
...
@overload
def device_put(value: mujoco.MjData, device_id: int) -> types.Data:
  ...


@overload
def device_put(value: mujoco.MjModel, device_id: int) -> types.Model:
  ...


def device_put(value, device_id: int):
  ...
  for f in dataclasses.fields(clz):  # type: ignore
    ...
    if f.type is jax.Array:
      field_value = jax.device_put(field_value, DEVICES[device_id])
    elif type(field_value) in _TYPE_MAP.keys():
      field_value = device_put(field_value, device_id)

    init_kwargs[f.name] = copy.copy(field_value)

  derived_kwargs = {}
  if isinstance(value, mujoco.MjModel):
    derived_kwargs = _model_derived(value, device_id)
  elif isinstance(value, mujoco.MjData):
    derived_kwargs = _data_derived(value, device_id)
