...
class SafeLayerBufferX:
    ...
    def get(self):
        ...
        data = dict(obs=self.obs_buf.view(self.env_num * self.max_ep_len, self.obs_buf.shape[-1])[valid], 
                    ...
                    prev_cost=self.prev_cost_buf.view(self.env_num * self.max_ep_len)[valid]
        )
        ...
        for t in range(max_ep_len):
           ...
           # step actions
            ...
            if timeout:
                ...
                # reset environment 
                o = env.reset()
                ep_ret, ep_len, ep_cost, prev_c = np.zeros(env_num), np.zeros(env_num, dtype=np.int16), np.zeros(env_num), torch.zeros(env_num, dtype=torch.float32).to(device)
