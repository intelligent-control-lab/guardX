...
def cpo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    def update():
        ...
        # determine if valid cost values are observed 
        if abs(EpCost) < 1e-20:
            print(colorize(f'No valid cost, continue with TRPO update.'%j, 'green', bold=True))
            
            # policy gradient direction for TRPO update 
            ...
            print(colorize(f'Valid cost detected, continue with CPO update.', 'green', bold=True))
            # solve QP
    ...
    for epoch in range(epochs):
        ...
        for t in range(max_ep_len):
            ...
            logger.store(TOther=t_other_step)   
        # Save model
        if ((epoch % save_freq == 0) or (epoch == epochs-1)) and model_save:
            logger.save_state({'env': env}, None)
        t_step = time.time() - t_epoch
        t_epoch = time.time()
        # Perform CPO update!
        update()
        t_update = time.time() - t_epoch
        #=====================================================================#
        ...
        logger.log_tabular('TOther', average_only=True)
