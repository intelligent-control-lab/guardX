...
class SafeLayerBufferX:
    ...
    def __init__(self, env_num, max_ep_len, obs_dim, act_dim, gamma=0.99, lam=0.95):
        ...
        self.prev_cost_buf = torch.zeros(env_num, max_ep_len, dtype=torch.float32).to(device)
    ...
    def store(self, obs, act, act_safe, rew, val, logp, mu, logstd, cost, prev_cost):
        ...
        self.prev_cost_buf[:,ptr] = prev_cost
        self.ptr += 1
    ...
    def get(self):
        ...
        data = dict(obs=self.obs_buf.view(self.env_num * self.max_ep_len, self.obs_buf.shape[-1]), 
                    ...
                    prev_cost=self.prev_cost_buf.view(self.env_num * self.max_ep_len)
        )
    ...
    for epoch in range(epochs):
        for t in range(max_ep_len):
           ...
           # step actions
            ...
            # save and log
            buf.store(o, a, a_safe, r, v, logp, mu, logstd, info['cost'], prev_c)
            ...
            if terminal:
                ...
                if timeout:
                    ...
                    # reset environment 
                    o = env.reset()
                    ep_ret, ep_len, ep_cost, prev_c, ep_cost_ret = np.zeros(env_num), np.zeros(env_num, dtype=np.int16), np.zeros(env_num), torch.zeros(env_num).to(device), np.zeros(env_num)
                    max_ep_len_ret = np.zeros(env_num)
                else:
