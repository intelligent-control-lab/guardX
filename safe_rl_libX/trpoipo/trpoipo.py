...
class TRPOIPOBufferX:
    ...
    def __init__(self, env_num, max_ep_len, obs_dim, act_dim, gamma=0.99, lam=0.95):
        ...
        self.epi_id_buf = torch.zeros(env_num, max_ep_len, dtype=torch.float32).to(device)
        self.gamma, self.lam = gamma, lam
        self.ptr = np.zeros(env_num, dtype=np.int16)
        self.path_start_idx = np.zeros(env_num, dtype=np.int16)
        self.epi_id = np.zeros(env_num, dtype=np.int16)
    ...
    def finish_path(self, last_val=None, done=None):
        ...
        if np.all(self.path_start_idx == 0) and np.all(self.ptr == self.max_ep_len):
            ...
            for done_env_idx in done_env_idx_all:
                ...
                self.epi_id_buf[done_env_idx, path_slice] = self.epi_id[done_env_idx]
                self.epi_id[done_env_idx] += 1
    ...
    def get(self):
        ...
        self.epi_id_buf[1:, :] += 1
        for i in range(1, self.epi_id_buf.shape[0]):
            self.epi_id_buf[i, :] += self.epi_id_buf[i-1, -1]
        ...
        data = dict(obs=self.obs_buf.view(self.env_num * self.max_ep_len, self.obs_buf.shape[-1]), 
                    ...
                    epi_id=self.epi_id_buf.view(self.env_num * self.max_ep_len)
        )
